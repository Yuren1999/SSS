...
当前已完成 `P0 工程骨架`、`P1 数据模型与接口`、`P2 本地谱线数据库`、`P3 HAPI 下载`、`P4 单气体合成`、`P5 混合气体与扰动模型`、`P6 基础批量合成与导出` 和 `P7 GUI 核心页面接入`，已包含：
...
HDF5 导出依赖 `h5py`，Parquet 导出依赖 `pandas` 和 `pyarrow`。这些依赖已写入项目依赖；如果是在旧环境中运行，需要重新执行上面的安装命令。
