...
def export_spectra(records: Sequence[SpectrumRecord], output_config: OutputConfig) -> Path:
    ...
    if output_config.output_format in {OutputFormat.HDF5, OutputFormat.PARQUET}:
        raise ExportError(f"{output_config.output_format.value} export requires a dedicated storage dependency")
                ...
                writer.writerow(
                    {
                        ...
                        "final_absorbance": _component_value(record.final_absorbance, point_index),
                        "transmittance": _component_value(record.transmittance, point_index),
                    }
                )
...
def _write_labels_csv(records: Sequence[SpectrumRecord], output_path: Path) -> None:
    ...
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for record in records:
            ...
            row.update(
                {f"variable.{name}": record.labels.variable_concentrations.get(name, "") for name in variable_names}
            )
            writer.writerow(row)
